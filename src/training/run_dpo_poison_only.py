...
OUTPUT_DIR   = "/wyh/wyh/VLM-Alignment-Probe/checkpoints/dpo_optimized"
...
# Training
# ---------------------------------------------------------------------------
def run_dpo(condition="poison_30pct", gpu=5, epochs=3, batch_size=1, grad_accum=16, beta=0.05, lr=1e-6):
...
def evaluate(adapter_path):
    ...
    results = {}
    for tag, use_adapter in [("base", False), ("dpo_poison_30pct", True)]:
        print(f"\n--- {tag} ---", flush=True)
    ...
    out_path = os.path.join(RESULTS_DIR, "dpo_optimized_results.json")
